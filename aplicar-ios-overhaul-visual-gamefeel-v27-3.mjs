...
function state(root){
  ...
  const checks = {
    app: has(path.join(root,rel.gameApp),['game_app_visual_v27_2','DailyMissionsV27_2','ShiftCommandDeckV27']),
    factoryClick: has(path.join(root,rel.studio),['factory_studio_v27_3','viewConfiguration.touchSlop','multiTouch','machineDialogId = touchedMachine.id']),
    factoryStations: has(path.join(root,rel.studio),['drawStationMarkerV27_2','"Q", "QUALIDADE"','"P", "PREPARAÇÃO"','"E", "EXPEDIÇÃO"']),
    workerSpeech: has(path.join(root,rel.studio),['val speaking = frame.workers','speechInterval','take(2)','studioWorkerQuoteV27(employee, worker.activity, phraseIndex)']),
    ...
    visual: has(path.join(root,rel.visual),['visual_experience_v27_2','DailyMissionsV27_2','CareerHowToProgressV27_2','TurnMetricCompactV27_2','Auto distribuir']),
...
function printState(s){Object.entries(s).forEach(([k,v])=>console.log(`[V27.3] ${k}: ${v?'OK':'PENDENTE'}`));}

const root=findRoot(process.cwd());
console.log('\n[V27.3] FÁBRICA VIVA + MISSÕES DIÁRIAS + REFINO UI/UX • Usinagem Master iOS/KMP');
console.log(`[V27.3] raiz: ${root}`);
const before=state(root);
if(CHECK){
  printState(before);
  const ok=Object.values(before).every(Boolean);
  if(!ok){console.error('[V27.3] CHECK FALHOU.');process.exit(2);}
  console.log('[V27.3] CHECK OK: toque, estações, falas, missões diárias, carreira e schema 4 validados.');
  const st=gitStatus(root,files.map(x=>x[1]));
  if(st){console.log('\n[V27.3] Alterações locais ainda não enviadas ao GitHub:');console.log(st);console.log('[V27.3] Faça commit + push antes do Actions remoto.');}
  process.exit();
}
if(DRY){console.log('[V27.3] DRY-RUN: instalaria V27.3 consolidada (12 Kotlin + 7 WAVs), com backup e schema 4 preservado.');printState(before);process.exit();}
if(Object.values(before).every(Boolean) && !FORCE){console.log('[V27.3] Já aplicada. Use --check para validar ou --force para reinstalar.');process.exit();}
...
const backupRoot=path.join(root,'.patch-backups','ios-overhaul-visual-gamefeel-v27-3',stamp());
...
console.log('\n[V27.3] APLICAÇÃO OK.');
console.log(`[V27.3] backup: ${backupRoot}`);
console.log(`[V27.3] arquivos com typo Roleta corrigido: ${typoCount}`);
console.log('[V27.3] Corrigido: toque em máquinas/operadores com gesto unificado.');
console.log('[V27.3] Corrigido: estações A/F/Q/P/C/E identificadas no galpão.');
console.log('[V27.3] Corrigido: falas independentes, lentas e no máximo 2 simultâneas.');
console.log('[V27.3] Corrigido: lista técnica com ações compactas.');
console.log('[V27.3] Dashboard: Precisão + Lucro + Foco como métricas principais.');
console.log('[V27.3] Novo: 3 missões diárias persistentes com dinheiro, ferramenta e XP do personagem.');
console.log('[V27.3] Carreira: painel explica exatamente como ganhar pontos industriais.');
console.log('[V27.3] GameSave permanece schema 4.');
const st=gitStatus(root,files.map(x=>x[1]).concat(audio.map(f=>path.join(rel.audioDir,f))));
if(st){console.log('\n[V27.3] IMPORTANTE — faça commit + push antes do GitHub Actions:');console.log(st);}
