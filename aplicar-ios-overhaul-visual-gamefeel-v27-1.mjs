...
function state(root){
  const checks = {
    ...
    compilerFixSpeechDrawText: has(path.join(root,rel.studio),['drawText(layout, topLeft = Offset(left+pad, top+pad*.7f))']) && !read(path.join(root,rel.studio)).includes('drawText(measurer, layout'),
  };
  return checks;
}
function printState(s){Object.entries(s).forEach(([k,v])=>console.log(`[V27.1] ${k}: ${v?'OK':'PENDENTE'}`));}

const root=findRoot(process.cwd());
console.log('\n[V27.1] OVERHAUL VISUAL + GAME FEEL • Kotlin/Native compile fix • Usinagem Master iOS/KMP');
console.log(`[V27.1] raiz: ${root}`);
const before=state(root);
if(CHECK){printState(before);const ok=Object.values(before).every(Boolean);if(!ok){console.error('[V27.1] CHECK FALHOU.');process.exit(2);}console.log('[V27.1] CHECK OK: V27.1 completa, correções Kotlin/Native presentes e save schema 4 preservado.');const st=gitStatus(root,files.map(x=>x[1]));if(st){console.log('\n[V27.1] Alterações locais ainda não enviadas ao GitHub:');console.log(st);console.log('[V27.1] Faça commit + push antes de rodar o Actions remoto.');}process.exit();}
if(DRY){console.log('[V27.1] DRY-RUN: instalaria a V27.1 completa (10 Kotlin + 7 WAVs) e as duas correções Kotlin/Native, com backup.');printState(before);process.exit();}
if(Object.values(before).every(Boolean) && !FORCE){console.log('[V27.1] Já aplicada. Use --check para validar ou --force para reinstalar.');process.exit();}
...
const backupRoot=path.join(root,'.patch-backups','ios-overhaul-visual-gamefeel-v27-1',stamp());
...
console.log('\n[V27.1] APLICAÇÃO OK.');
console.log(`[V27.1] backup: ${backupRoot}`);
console.log(`[V27.1] arquivos com typo Roleta corrigido: ${typoCount}`);
console.log('[V27.1] Entregas preservadas da V27 + correção BorderStroke + correção drawText(TextLayoutResult) para Kotlin/Native.');
console.log('[V27.1] GameSave schema 4 e Cloud Save não foram alterados.');
const st=gitStatus(root,files.map(x=>x[1]).concat(audio.map(f=>path.join(rel.audioDir,f))));
if(st){console.log('\n[V27.1] IMPORTANTE — faça commit + push antes do GitHub Actions:');console.log(st);}

console.log('[V27.1] Compile-fix: BorderStroke importado em AndroidV24ParityUi.kt e balão de fala usa drawText(TextLayoutResult).');
